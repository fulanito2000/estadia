...
$same_page = "mesa/lista_ticket_pakal.php";
...
$pantalla_edicion = "compras/crearCamion.php";

$permiso_detalle_servicio = $parametroGlobal->getRegistroById(29) && $parametroGlobal->getActivo() == "1";
...
if(!empty($negocios)){
    ...
    $Where .= " t.ClaveCliente IN($string_clientes) ";
}
...
<html lang="es">
    ...
    <body>
        <div class="p-4 rounded"> <!-- fondo del contenido -->
            <div class="principal">            
                <form action="reportes/ReporteTicketsExcel.php" method="post" target="_blank" id="FormularioExportacion">
                    <div class="form-row">
                        <div class="form-group col-md-4">
                            <label for="busqueda_ticket"><?php echo $nombre_objeto; ?></label>
                            <input class="form-control" id="busqueda_ticket" name="busqueda_ticket" value="<?php echo $idTicket; ?>"/> 
                            <p id="error_busqueda_ticket text-danger" style="display: none;">Ingresa s&oacute;lo n&uacute;meros por favor</p> 
                        </div>
                        <div class="form-group col-md-4">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input id="fecha_fin" name="fecha_fin"  class="form-control" value="<?php echo $FechaFin; ?>"/>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-check col-md-4">
                            <p>Filtrar:</p>
                            <input type="checkbox" id="ticket_cerrado" name="ticket_cerrado" <?php echo $checked; ?> /><label for="ticket_cerrado"><?php echo $nombre_objeto; ?>es cerrados</label>
                            <input type="checkbox" id="ticket_cancelado" name="ticket_cancelado" <?php echo $checkedCancelado; ?> />
                            <label for="ticket_cancelado"><?php echo $nombre_objeto; ?>es cancelados</label>
                        </div>
                        <div class="form-group col-md-4">
                            <label for="estadot">Estado</label>
                            <select id="estadot" name="estadot" class="custom-select">
                                <?php
                        ...
                        <div class="form-group col-md-4">
                            <label for="estado_ticket">SubEstado</label>
                            <select id="estado_ticket" name="estado_ticket" class="custom-select">
                                <?php
                                /* Inicializamos la clase */
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            echo "<select id='prioridad' name='prioridad' class='custom-select w-75'>";
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <select id="cliente_ticket" class="custom-select" name="cliente_ticket[]" multiple>
                                <?php
                                if(!empty($negocios)){
                                    $query = $catalogo->obtenerLista("SELECT NombreRazonSocial AS cliente FROM `c_cliente` WHERE ClaveCliente IN($string_clientes) ORDER BY cliente;");
                                }else if (empty($array_clientes)) {//$clientes_permitidos
                                ...
                                while ($rs = mysql_fetch_array($query)) {
                                    ...
                                    if (isset($_POST['cliente']) && $_POST['cliente'] != "" && in_array($rs['cliente'], $cliente_array)) {
                                        $s = "selected='selected'";
                                    }else if(isset ($_GET['cliente']) && $_GET['cliente'] != ""){//Es el único que hay
                                        $s = "selected='selected'";
                                    }
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-4">
                            <input type="button" class="btn btn-success" onclick="recargarListaTicket('mesa/lista_ticket_pakal.php', 'ticket_cerrado', 'cliente_ticket', 'ticket_color',
                                    ...
                                    return false;" id="boton_aceptar" name="boton_aceptar" value="Crear <?php echo $nombre_objeto; ?>"/>
                        </div>
                        <?php if(isset($_GET['regresar']) && $_GET['regresar'] != ""){ ?>
                            <div class="form-group col-md-4">
                                <input type='button' class='btn btn-danger' title='Regresar' value='Regresar' onclick='cambiarContenidos("<?php echo $_GET['regresar']?>","")'>
                            </div>
                        <?php } ?>
                        <?php if ((isset($_POST['mostrar']) && $_POST['mostrar'] == "true") || (isset($_GET['mostrar']) && $_GET['mostrar'] == "true")) { ?>
                            <div class="form-group col-md-4">
                            <button class="btn btn-info" title="Exportar a excel" id="excelSubmit" name="excelSubmit" value="Exportar a excel" onclick="submitform()">Exportar a excel <i class="fal fa-file-excel"></i></button>
                ...
                <?php
                if ((isset($_POST['mostrar']) && $_POST['mostrar'] == "true") || (isset($_GET['mostrar']) && $_GET['mostrar'] == "true") || $idTicket != "") {/* Si se quiere mostrar el grid */
                if ($permiso_detalle_servicio) {
                    ?>

                <table>
                        <tr>
                            <td>
                                <a href="mesa/DetalleServicios.php<?php echo $urlextra ?>" target="_blank" class="btn btn-warning">Generar excel</a>
                            </td>
                        </tr>
                </table> <?php } ?>
                <div class="reporte table-responsive dataTable">
                <table id="tAlmacen" class="table" >
                    ...
                    <thead>
                        <tr>
                            <?php
                            $cabeceras = array("Camion", "Fecha", /*$Titulo_NoSerie,*/ "Cliente", /*$TituloTablaAreaAtencion,*/ /*$TituloTablaZona,*/ "Contenido", "Ultimo estatus Camion", "Última Nota", "Fecha nota", "Personal",/* $TituloTablaGuia,*/ /*"Día atraso",*/ "", "", "");
                            ...
                            for ($i = 0; $i < (count($cabeceras)); $i++) {
                                echo "<th align=\"center\" style=\"width:1%;color:black;\" scope=\"col\">" . $cabeceras[$i] . "</th>";
                            }
                            ?>                        
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        if ($idTicket == "") {
                            $consulta = "SELECT
                                ...
                                FROM c_ticket AS t
                                INNER JOIN c_estadoticket AS e ON $tipoReporte $canceladoTicket $cerradoTicket e.IdEstadoTicket = t.EstadoDeTicket $cliente
                                ...
                                $tipo_join_estado JOIN c_estado AS e2 ON e2.IdEstado = t.AreaAtencion                                
                                $estadoNota
                            ...
                            if (is_numeric($idTicket)) {
                                $consulta.=" WHERE t.Activo = 1 AND (t.IdTicket = $idTicket OR NoTicketCliente = '$idTicket' OR NoTicketDistribuidor = '$idTicket') ";
                            } else {
                                $consulta.=" WHERE t.Activo = 1 AND (NoTicketCliente = '$idTicket' OR NoTicketDistribuidor = '$idTicket') ";
                            }
                        ...
                        while ($rs = mysql_fetch_array($query)) {/* Recorremos todos los tickets resultantes del query */
                            ...
                            if ($rs['Resurtido'] == '1') {
                                ...
                                //echo "<td data-toggle='tooltip' title='" . $rs['DatosUsuario'] . "' align='center' scope='row'><a href='#' onclick='cambiarContenidos(\"almacen/alta_bitacora.php?consulta_tiquet=mesa/lista_ticket_pakal.php&NoSerie=" . $rs['NumSerie'] . "\"); return false;'>" . $rs['NumSerie'] . "</a></td>";
                            } else {
                                $series = explode(",", $rs['NumSerie']);
                                $texto = "";
                                /*foreach ($series as $value) {
                                    $texto.= "<a href='#' onclick='cambiarContenidos(\"almacen/alta_bitacora.php?consulta_tiquet=mesa/lista_ticket_pakal.php&NoSerie=$value\"); return false;'>" . $value . "</a>,";
                                }*/
                                ...
                                <?php if ($permisos_grid->getModificar()) { ?>
                                    ...
                                    if ($booleanFecha) {
                                        ...
                                        <a href='#' onclick='editarTicket("<?php echo $pantalla_edicion; ?>", "<?php echo $rs['IdTicket']; ?>", "<?php echo $rs['TipoReporte']; ?>", "0");
                                                                    return false;' title='Modificar' class="text-warning" style="font-size: 1.5rem;"><i class="fal fa-pencil-alt"></i></a>
                                       <?php } ?>  
                                   <?php } ?>
                            </td>                            
                            <td align='center' scope='row'>
                                <?php if ($permisos_grid->getModificar()) { ?>
                                    <a href='#' onclick='AgregarNotaTicketLista("nota","AgregarNota",<?php echo $rs['IdTicket']; ?>,"");
                                                            return false;' title='Agregar nota' style="color: peru;font-size: 1.5rem;">
                        ...
                        if ($tiene_permisoTicket) {
                            if ((int) $rs['Resurtido'] == 1) {  //104 en produccion, 201 pruebas
                                ...
                                ?>
                                <td align='center' scope='row'>                             
                                    <a href='#' onclick='detalleReporte("reportes/reporte_ticket_pakal.php", "<?php echo $rs['IdTicket']; ?>", "<?php echo $rs['idArea']; ?>", "0");
                        ...
                        if ($rs['IdEstatusAtencion'] == "71") {
                            ?>
                            <td align='center' scope='row'>                             
                                <a href='#' onclick='cambiarContenidos("ventas/alta_orden_compra_pakal.php?ticket=<?php echo $rs['IdTicket']; ?>", "Compras > Abrir Camion", "0");
                                                    return false;' title='Abrir Camion' class="text-dark" style="font-size: 1.5rem;"><i class="far fa-truck-loading"></i></a>

                            </td>
                            <?php
                            /* <td align='center' scope='row'>                             
                                <a href='#' onclick='cambiarContenidos(<?php echo 'ventas/NuevaPrecioABC_pakal.php?ticket='.$rs['IdTicket']; ?>);
                                    return false;' title='Abrir Camion' ><img src="resources/images/key.png" width="50" height="50"/></a>
                        ...
                        echo "<td scope='row' style='text-align: center;'><a onclick='eliminarCamion(".$rs['IdTicket']."); return false;' style='cursor: pointer;font-size:1.5rem;' class='text-danger'><i class='fal fa-trash'></i></a></td>";
                        echo "</tr>";
                    }
                    ?>
                    </tbody>

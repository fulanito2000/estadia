...
$paginaActual = "almacen/lista_refaccionesSolicitadas.php";
$idTicket = "";
$cliente = "";
$color = "";
$colorPOST = "";
...
if(isset($_GET['erefaccion']) && (int)$_GET['erefaccion'] == 1){
    $paginaActual.="?erefaccion=1";
}else if(isset($_GET['srefaccion']) && (int)$_GET['srefaccion'] == 1){
    $paginaActual.="?srefaccion=1";
}
        ...
        <div class="principal">  
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-3">
                    <label for="cliente_ticket">Cliente</label>
                      <select id="cliente_ticket" name="cliente_ticket"  class="filtro custom-select">
                            <?php
                            ...
                            while ($rs = mysql_fetch_array($query)) {
                                ...
                                if (isset($_POST['cliente']) && $_POST['cliente'] == $rs['ClaveCliente']) {
                                    $s = "selected='selected'";
                                }else if(isset($_GET['cliente']) && $_GET['cliente'] == $rs['ClaveCliente']){
                                    $s = "selected='selected'";
                                }
                ...
                <div class="form-group col-md-3">
                    <label for="slcAreaAtencion">Área de atención</label>
                      <select id="slcAreaAtencion" name="slcAreaAtencion" class="custom-select">
            ...
            <div class="form-row">
                <div class="form-group col-md-2">
                    <input type="checkbox" name="verCerrado" id="verCerrado" <?php echo $cerrados ?>/>
                    <label for="verCerrado">Ver cerrados</label>
                </div>
                <div class="form-group col-md-2">
                    <label for="">Ver cancelados</label>
                    <input type="checkbox" name="verCancelado" id="verCancelado" <?php echo $cancelados ?>/>
                </div>
                <div class="form-group col-md-2">
                    <label for="verMoroso">Ver Morosos</label>
                    <input type="checkbox" name="verMoroso" id="verMoroso" <?php echo $moroso ?>/>
                </div>
                <div class="form-group col-md-2">
                    <label for="verEnviados">Ver enviados</label>
                    <input type="checkbox" name="verEnviados" id="verEnviados" <?php echo $enviados; ?>/>
                </div>
                <div class="form-group col-md-3">
                    <input type="Button" id="boton_aceptar" name="boton_aceptar" value="Buscar ticket" class="btn btn-success btn-block" onclick="BuscarTicket('<?php echo $paginaActual; ?>');"/>
                    <?php if(isset($_GET['regresar']) && $_GET['regresar'] != ""){?>
                        <input type="button" class="btn btn-warning" value="Regresar" onclick="cambiarContenidos('<?php echo $_GET['regresar']?>','')">
                        <?php }?>
                </div>
            </div>   

            <?php if ((isset($_POST['mostrar']) && $_POST['mostrar'] == "true") || (isset($_GET['mostrar']) && $_GET['mostrar'] == "true")) { 
                    if(!isset($_GET['erefaccion']))
                    {
                        if(!isset($_GET['srefaccion'])){
                            echo "<h1 class='p-2'>Refacciones solicitadas</h1>";
                        }
                if ($permisos_grid->getModificar()) {
                    ?>
                    <input type="button" class="btn btn-outline-success" id="guardar_seleccionados" value="Guardar seleccionados" onclick="guardarMultiplesSeleccionadas('<?php echo $paginaActual; ?>');
                ...
                <?php 
                    if(isset($_GET['srefaccion']) && (int)$_GET['srefaccion'] == 1){
                    ...
                    <table id="tAlmacen" class="tabla_grid" style="display: none; width: 100%;">
                        ...
                        <tr>
                            ...
                            $cabeceras = array("Ticket", "Fecha", "NoSerie", "Cliente", "Falla", "", "Área Atención", "Refacción", "Último Estado",
                                "", "", "Almacén", "", "Estatus", "", "", "","Estado del componente");
                        ...
                        if ($idTicket == "") {                            
                            ...
                            (SELECT nr.EstadoComponente FROM k_nota_refaccion nr WHERE nr.NoParteComponente = c.NoParte AND (nr.EstadoComponente = 'CU' OR nr.EstadoComponente = 'CN')limit 1) AS EdoComp,                             
                                ...
                                LEFT JOIN c_estado e ON lastnt.IdEstatusAtencion = e.IdEstado
                                ...
                                AND $cliente nr.Cantidad<>0 AND c.IdTipoComponente=1 
                        ...
                        while ($rs = mysql_fetch_array($query)) {/* Recorremos todos los tickets resultantes del query */
                            ...
                            foreach ($series as $value) {
                                ...
                                cambiarContenidos(\"almacen/alta_bitacora.php?consulta_tiquet=$paginaActual&NoSerie=$value\"); 
                            ...
                            onclick='BuscarRefaccionesByTicket(\"" . $rs['IdTicket'] . "\",\"".$paginaActual."\"); return false;'>" . $rs['IdTicket'] . "</a></td>";
                            echo "<td align='center' scope='row' style='font-size:10px'>" . $rs['FechaHora'] . " (". $diasCreacion. " días ".$diffT ." horas)". "</td>";
                        ...
                        echo "<td align='center' scope='row' style='font-size:10px'>" . $rs['areaAtencion2'] . " (".$rs['areaAtencion'].")</td>";
                        ...
                        echo "</td>";
                        echo "<td align='center' scope='row' style='font-size:10px'>" . $rs['ultimoEstatus'] . " (". $diasUltimaModificacion ." días ".$diffUT ." horas) </td>";
                        ?>
                        ...
                        if ($permisos_grid->getModificar() && $ticket_abierto) {
                            ...
                            <input type="button" class="boton" id="guardar_refaccion_<?php echo $contadorFila; ?>" value="Guardar" onclick="CambiarEstatusRefaccion('<?php echo $contadorFila ?>', '<?php echo $rs['Cantidad'] ?>',
                                    ...
                                    '<?php echo $rs['IdEstatusCobranza'] ?>', true, '<?php echo $paginaActual; ?>' );"/> 
                               <?php } ?>
                               <?php
                                echo "</td>";
                                echo "<td>";
                        ...
                        <?php
                        /*$sql=("SELECT k_nota_refaccion.EstadoComponente 
                                                FROM c_componente
                                                INNER JOIN k_nota_refaccion
                                                ON  c_componente.NoParte=k_nota_refaccion.NoParteComponente
                                                WHERE k_nota_refaccion.EstadoComponente='CU' OR k_nota_refaccion.EstadoComponente='CN'");*/
                       
                        ?>
                        <!--<td align='center' scope='row' style='font-size:10px'><?php echo $sql; ?></td>-->
                        <?php 
                        //Atahu
                       echo "<td align='center' scope='row' style='font-size:10px'>".$rs['EdoComp']."</td>";
                        echo "</tr>";
                        $contadorFila++;
                        }
                        ?>
                ...
                <?php if ($permisos_grid->getModificar()) {
                    ...
                    if(!isset($_GET['srefaccion'])){
                        echo "<br/><br/>";
                        if(!isset($_GET['erefaccion'])){
                            echo "<h1>Env&iacute;o de refacciones</h1>";
                        }
                if ($permisos_grid->getModificar()) {
                    ?>
                    <input type="button" id="enviar" name="enviar" onclick="picking('<?php echo $paginaActual; ?>');
                        return false;" class="btn btn-outline-primary" style="float: right;" value="Entregar"/>
                <?php } ?>
                <br/>
                <?php if(isset($_GET['erefaccion'])){ 

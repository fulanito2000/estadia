        ...
        <div class="principal">
            <div class="form-row">
                <div class="form-group col-md-3">
                    <label for="almacen_filtro">Almacén: </label>
                    <select id="almacen_filtro" name="almacen_filtro[]" class="multiselect custom-select" multiple="multiple">
                            <?php                                
                ...
                <div class="form-group col-md-3 offset-md-6 p-3 pl-3">
                    <input type="button" class="btn btn-success btn-block" onclick="mostrarEntradasDeEquipo('<?php echo $same_page; ?>','almacen_filtro'); return false;" 
            ...
            <?php if(isset($_POST['mostrar']) && $_POST['mostrar'] == "true"){ ?>
                ...
                <div class="table-responsive p-2">
                    <table id="tAlmacen" style="max-width: 99">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        $consulta = "SELECT meq.id_movimientos, meq.NoSerie, meq.Fecha, a.id_almacen, b.id_bitacora, b.NoParte, e.Modelo,
                            ...
                            WHERE meq.pendiente = 1 AND kae.id_almacen = 9 AND !ISNULL(meq.almacen_nuevo) AND (!ISNULL(clave_cliente_anterior) OR !ISNULL(almacen_anterior)) 
                            AND meq.id_movimientos = (SELECT MAX(id_movimientos) 
                            FROM movimientos_equipo WHERE NoSerie = meq.NoSerie AND pendiente = 1 AND !ISNULL(almacen_nuevo))                            
                            $filtro_responsable_almacen GROUP BY NoSerie ";
                        ...
                        $consulta .= " UNION
                            ...
                            c_equipo.Modelo ,meq.almacen_nuevo AS id_nuevo, '' AS destino, '' AS origen,
                            ...
                            LEFT JOIN c_solictudretirogeneral AS csrg ON csrg.IdSolicitudRetiroGeneral=csr.IdSolicitudRetiroGeneral
                            LEFT JOIN movimientos_equipo AS meq ON meq.id_movimientos = (SELECT id_movimientos FROM movimientos_equipo WHERE NoSerie = c_bitacora.NoSerie AND !ISNULL(almacen_nuevo) LIMIT 1)
                            WHERE id_almacen = 9 
                        ...
                        $consulta .= " ORDER BY id_movimientos DESC";
                        //echo $consulta;
